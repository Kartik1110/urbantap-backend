import { z } from "zod";
import { City, Category, Bedrooms, Bathrooms, Furnished, Payment_Plan } from "@prisma/client";
...
export const createProjectSchema = z.object({
  ...
  type: z.nativeEnum(Category, {
    errorMap: () => ({ message: `Type must be one of: ${Object.values(Category).join(', ')}` })
  }),
  ...
  furnished: z.nativeEnum(Furnished, {
    errorMap: () => ({ message: `Furnished must be one of: ${Object.values(Furnished).join(', ')}` })
  }),
  ...
  unit_types: z.preprocess(
    ...
    z.array(z.string().min(1, "Unit type cannot be empty"))
      .max(50, "Too many unit types (max 50)")
  ),

  amenities: z.preprocess(
    (val) => typeof val === "string" ? JSON.parse(val) : val,
    z.array(z.string().min(1, "Amenity cannot be empty"))
