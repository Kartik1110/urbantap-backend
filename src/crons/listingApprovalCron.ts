...
async function approveListings(): Promise<void> {
    try {
        ...
        logger.info(
            `Successfully processed ${pendingListings.length} pending listings`
        );
    } catch (error) {
        logger.error(
            'Error in approveListings cron job:',
...
async function sendHourlyListingNotifications(): Promise<void> {
    ...
    logger.info(
        `Hourly listing notification sent to ${brokers.length} brokers`
    );
}
