...
import { bulkInsertBrokersSchema, updateBrokerDataSchema } from "../schema/broker.schema";
...
export const bulkInsertBrokers = async (req: Request, res: Response) => {
  ...
  try {
    ...
    bulkInsertBrokersSchema.parse({ brokers });
  } catch (error) {
    return res.status(400).json({
      status: "error", 
      message: "Invalid brokers data",
...
export const updateBroker = async (req: Request, res: Response) => {
  ...
  try {
    ...
    updateBrokerDataSchema.parse(updateData);
  } catch (error) {
    return res.status(400).json({
      status: "error",
