...
import { NotificationType, OrderType } from '@prisma/client';
import {
...
export const getAdminListingsService = async (filters: {
    search?: string;
    looking_for?: boolean;
...
export const updateListingStatusService = async (
    ...
    if (brokerUser?.fcm_token) {
        ...
        try {
            ...
            console.error(
                '❌ Failed to send notification to broker user:',
                brokerUser.id,
                error
            );
        }
        ...
        console.warn(
            '⚠️ No FCM token found for listing owner:',
            brokerUser?.id
        );
    }
    ...
    if (status === 'Approved') {
        ...
        const listingType =
            updatedListing.sale_type?.toLowerCase() || 'property';
        const location =
            updatedListing.address || updatedListing.city || 'a location';
...
export const assignCreditsToCompany = async ({
    company_id,
    credits,
