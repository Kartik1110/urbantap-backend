...
export async function uploadToS3Chunked(
    ...
    try {
        ...
        logger.info(
            `🔄 Upload initialized with ${Math.ceil(fileStats.size / chunkSize)} expected parts`
        );
        ...
        upload.on('httpUploadProgress', (progress) => {
            if (progress.loaded && progress.total) {
                ...
                logger.info(
                    `📈 Upload progress: ${percentage}% (${(progress.loaded / (1024 * 1024)).toFixed(2)}MB / ${(progress.total / (1024 * 1024)).toFixed(2)}MB)`
                );
            }
        });
        ...
        await upload.done();
        logger.info(
            `✅ Chunked upload completed successfully for: ${fileName}`
        );
