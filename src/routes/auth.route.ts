...
import {signupSchema,loginSchema,googleSignInSchema,appleSignInSchema,updateUserSchema,} from "../schema/auth.schema"; 

const router = Router();

// Mobile app routes
router.post("/google/signin",validateSchema(googleSignInSchema), googleSignIn);
router.post("/apple/signin",validateSchema(loginSchema), appleSignIn);
