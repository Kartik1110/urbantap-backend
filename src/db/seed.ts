...
async function main() {
    ...
    for (let i = 0; i < 60; i++) {
        ...
        if (isDeveloper) {
            ...
            // <<< ADD THIS: Link company to developer >>>
            await prisma.company.update({
                where: { id: company.id },
                data: { developerId: developer.id },
            });

            // 4a. Create Broker for Developer
            ...
            // <<< ADD THIS: Link company to brokerage >>>
            await prisma.company.update({
                where: { id: company.id },
                data: { brokerageId: brokerage.id },
            });

            // 4b. Create Broker for Brokerage
            const broker = await prisma.broker.create({
                data: {
                    name: user.name!,
                    email: user.email,
